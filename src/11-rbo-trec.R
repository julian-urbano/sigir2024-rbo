...
for(year in .WEB_YEAR) {
  ...
  # Read runs
  d <- read_inputs(glue("scratch/01-trec-download/web{year}/"), breaker = "random", topk = 1000)
  ...
  r <- future_lapply(1:nrow(r), function(i) {
    ...
    # sorted by docID
    run1docid <- run1 |> arrange(desc(score), doc)
    run2docid <- run2 |> arrange(desc(score), doc)
    ...
    if(all(!is.na(run1$score)) && all(!is.na(run2$score)) &&
      ...
      lapply(.P, function(p) {
        ...
        data.frame(sys1, sys2, topic = top, p = p,
                   rbo_rand = rbo(run1$doc, run2$doc, p, "w", "ext"),
                   rbo_docid = rbo(run1docid$doc, run2docid$doc, p, "w", "ext"),
